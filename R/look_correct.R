...
#lam_0 <- 1e-04          #annual risk of TB infection, per smear-positive active case
...
fp_neg <- 0.45     #Fraction of incident TB that is smear-positive
fp_hiv <- 0 	#.45
fp_aids <- 0 		#0.35
...
pp_neg <- 0.07     #proportion of TB infections causing immediate progression to active TB
pp_hiv <- 0        #0.07
pp_aids <- 0       #0.56

lr_neg <- 0.28       #proportion of TB reinfections capable of primary progression
lr_hiv <- 0 #.28
lr_aids <- 0 #0.75
...
hivinc <- 0       #annual risk of HIV infection (HIV incidence)
...
sn_smpos   <- 0.8   #sensitivity of diagnosis, smear-positive
sn_smneg  <- 0.25   #sensitivity of diagnosis, smear-negative
...
DSP_neg.init <- pop/24     #smear-positive patients with access to TB diagnostic sevices
USP_neg.init <- pop/24   #smear-positive patients without access to TB diagnostic services
DSP_hiv.init <- 0
USP_hiv.init <- 0
DSP_aids.init <- 0
USP_aids.init <- 0

DSN_neg.init <-   pop/24 #smear-negative patients with access to TB diagnostic sevices
USN_neg.init <-  pop/24   #smear-negative patients without access to TB diagnostic services
DSN_hiv.init <- 0
USN_hiv.init <- 0
DSN_aids.init <- 0
USN_aids.init <- 0
...
S_neg.init <-   pop/2    #susceptible
S_hiv.init <- 0
S_aids.init <- 0
L_neg.init <-  pop/3    #latently-infected
L_hiv.init <- 0
L_aids.init <- 0
...
#C
#the force of infection (annual rate of TB infection) at time t
	lam[end.step] <- x*(TSP[end.step]+i_smneg*TSN[end.step])
...
return(lam[end.step]-0.02)


}

#look(1e-5)
#solution <- uniroot(f=look, interval=c(1e-4, 1e-3), tol = 1e-5)

equilib <- c(S_neg[end.step],L_neg[end.step], DSP_neg[end.step],USP_neg[end.step],DSN_neg[end.step],USN_neg[end.step],lam[end.step], )
